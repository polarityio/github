...
const SEARCH_ENGINE_ID = '6f8ac4b3904cb7c1a';
...
function _createQuery(entity, searchFilters) {
  ...
  let query = `"${entity.value.replace('exf:','').replace('=',' ').trim()}"`;
    ...
    results.forEach((result) => {
      if (_isMiss(result.body)) {
        ...
        lookupResults.push({
          ...
          data: {
            summary: [`${result.body.searchInformation.totalResults} public posts`],
